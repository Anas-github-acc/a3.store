...
```
    Node 1                    Node 2                    Node 3
       │                         │                         │
       │──── Heartbeat {id:1, hb:42} ────►│                │
       │                         │──── Heartbeat {id:2, hb:17} ────►│
       │◄──── Heartbeat {id:3, hb:89} ────│◄────────────────────────│
       │                         │                         │
    ┌──┴──┐                   ┌──┴──┐                   ┌──┴──┐
    │Membership Table         │Membership Table         │Membership Table
    │ node-1: hb=42           │ node-1: hb=42           │ node-1: hb=42
    │ node-2: hb=17           │ node-2: hb=17           │ node-2: hb=17
    │ node-3: hb=89           │ node-3: hb=89           │ node-3: hb=89
    └─────┘                   └─────┘                   └─────┘
```
...
```
  Client                Node 1 (Primary)           Node 2 (Replica)
     │                        │                          │
     │─── PUT(key, value) ───►│                          │
     │                        │── Write to SQLite        │
     │                        │                          │
...
```
    ...
    │                    Anti-Entropy Loop                        │
    │                                                             │
    │  1. Split keyspace into 16 chunks                           │
    │                                                             │
    │  2. For each chunk:                                         │
    │     ┌──────────────────────────────────────────────────┐    │
    │     │  local_hash = SHA256(chunk_data)                 │    │
    │     │  peer_hash  = RPC GetChunkHash(peer, chunk_id)   │    │
    │     │                                                  │    │
    │     │  if local_hash ≠ peer_hash:                      │    │
    │     │      data = RPC FetchRange(peer, chunk_id)       │    │
    │     │      for each (key, value, timestamp) in data:   │    │
